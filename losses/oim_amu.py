...
class OIM(autograd.Function):
    ...
    def backward(ctx, grad_outputs):
        ...
        for x, y in zip(inputs, targets):
            if y < len(lut):
                if lut[y].count_nonzero() == 0:
                    # if empty, just put the input
                    lut[y] = x
                    lut[y] /= lut[y].norm()
                else: 
                    # adaptive momentum
                    sim_self = torch.sum(x*lut[y])

                    sim = torch.mm(lut, x.view(-1,1)).view(-1)
                    sim, q = torch.topk(sim, 2)
                    sim_max = sim[0]
                    if q[0] == y: sim_max = sim[1]

                    momentum = torch.exp(sim_max/temp) / (torch.exp(sim_self/temp)+torch.exp(sim_max/temp))
